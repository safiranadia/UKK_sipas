...
@section('content')
    ...
    <div class="flex-1 overflow-auto p-8">
        ...
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-gray-900 font-primary">Manajemen Pengaduan</h2>
            <a href="{{ route('admin.reports.exportPDF') }}?{{ http_build_query(request()->query()) }}"
                class="flex items-center gap-2 px-4 py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition-all shadow-md">
        ...
        <form method="GET" action="{{ route('admin.reports.index') }}"
            ...
            <div class="md:col-span-3">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari laporan / nama user"
        ...
        @if ($pendingReports->count() > 0)
            <div class="mb-10">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <span
                        class="px-3 py-1 bg-danger text-white rounded-full text-xs font-bold">{{ $pendingReports->count() }}
                        Laporan</span>
                </div>
                        ...
                        <div
                            ...
                            <div class="relative h-48 overflow-hidden">
                                ...
                                <div class="absolute top-3 right-3">
                                    <span
                                        class="px-2.5 py-1 bg-danger text-white text-[10px] font-bold uppercase tracking-wider rounded-lg shadow-sm">
                                        Baru
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="p-5 flex-1 flex flex-col">
                                ...
                                <!-- Facility Info -->
                                <div class="mb-3">
                                    <h3 class="text-sm font-bold text-gray-900 line-clamp-1 mb-1">
                                        {{ $report->nama_fasilitas }}
                                    </h3>
                                    <p class="text-[11px] text-blue-600 font-semibold mb-2">
                                        {{ $report->categoryReport->nama_kategori ?? 'Fasilitas' }}
                                    </p>
                                    <p class="text-xs text-gray-500 line-clamp-2 leading-relaxed">{{ $report->deskripsi }}
                                    </p>
                                </div>
        ...
        <div>
            <div class="flex items-center gap-3 mb-6">
                ...
                <span
                    class="px-3 py-1 bg-blue-600 text-white rounded-full text-xs font-bold">{{ $processedReports->count() }}
                    Laporan</span>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        ...
                        <div class="relative h-48 overflow-hidden">
                            ...
                            <div class="absolute top-3 right-3">
                                @php
                                    ...
                                    $currentStatus = $statusMap[$report->status] ?? [
                                        'label' => $report->status,
                                        'color' => 'bg-gray-400',
                                    ];
                                @endphp
                                <span
                                    class="px-2.5 py-1 {{ $currentStatus['color'] }} text-white text-[10px] font-bold uppercase tracking-wider rounded-lg shadow-sm">
                                    {{ $currentStatus['label'] }}
                                </span>
                            </div>
                        </div>
                            ...
                            <div class="mt-auto pt-4 flex flex-col gap-3 border-t border-gray-50">
                                ...
                                <div class="grid grid-cols-1 gap-2">
                                    @if ($report->status == 'approved')
                                        <form id="status form-{{ $report->id }}" method="POST"
                                            action="{{ route('admin.reports.updateStatus', $report->id) }}">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="status" value="in_progress">
                                            <button type="submit"
                                                class="w-full px-4 py-2 bg-warning text-white rounded-lg text-xs font-bold hover:bg-warning/90 transition-all">
                                                Tandai Diproses
                                            </button>
                                        </form>
                                    @elseif($report->status == 'in_progress')
                                        <form id="status form-{{ $report->id }}" method="POST"
                                            ...
                                            <button type="submit"
                                                class="w-full px-4 py-2 bg-success text-white rounded-lg text-xs font-bold hover:bg-success/90 transition-all">
                                                Tandai Selesai
                                            </button>
                                        </form>
                                    @elseif($report->status == 'resolved')
                                        <x-modal-solve :report="$report" isAdmin="true" variant="button"
                                            triggerText="Lihat Solusi" />
                                    @endif
                                </div>
                            </div>
                        </div>
                ...
                @empty
                    <div class="col-span-full py-12 text-center bg-white rounded-2xl border border-dashed border-gray-200">
                        <p class="text-gray-500">Belum ada laporan yang sudah diproses.</p>
                    </div>
                @endforelse
            </div>
        </div>
