...
<div id="modal-progres-{{ $report->id ?? 'default' }}" x-data="{
    ...
    @open-modal-progress-{{ $report->id ?? 'default' }}.window="open = true" @keydown.escape.window="open = false"
    x-cloak>
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div
                ...
                <div class="px-6 py-6 max-h-[85vh] overflow-y-auto">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                        ...
                        <div class="bg-gray-50 rounded-lg p-3">
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ $report->user->username ?? 'Unknown' }}
                            </p>
                        </div>
                        <!-- Status -->
                        <div class="bg-gray-50 rounded-lg p-3">
                        ...
                        <div class="bg-gray-50 rounded-lg p-3">
                            ...
                            <p class="text-sm font-medium text-gray-700">
                                {{ $report->tanggal_laporan ? \Carbon\Carbon::parse($report->tanggal_laporan)->format('d/m/y') : '-' }}
                            </p>
                        </div>
                        <!-- Fasilitas -->
                        <div class="bg-gray-50 rounded-lg p-3">
                    ...
                    <div x-data="{ showDetail: false }" class="mb-6">
                        <button @click="showDetail = !showDetail"
                            class="flex items-center justify-between w-full p-3 bg-gray-50 rounded-lg text-sm font-semibold text-gray-900 outline-none">
                            ...
                            <svg class="w-4 h-4 transition-transform duration-200"
                                :class="showDetail ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7" />
                            </svg>
                        ...
                        <div x-show="showDetail" x-collapse
                            class="p-4 border border-gray-100 rounded-b-lg mt-1 space-y-4">
                            <div>
                    ...
                    <div class="relative py-4">
                        ...
                        <div class="relative flex justify-center text-sm">
                            <span
                                class="px-3 bg-white text-gray-500 font-bold uppercase tracking-wider text-[10px]">Diskusi
                                Laporan</span>
                        </div>
                    </div>
                    ...
                    <!-- Comment Section -->
                    <div
                        class="flex flex-col h-80 bg-gray-50 rounded-xl overflow-hidden border border-gray-100 relative">
                        {{-- Comment List --}}
                        <div x-ref="commentContainer" class="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth">
                            ...
                            <template x-for="comment in comments" :key="comment.id">
                                <div
                                    :class="comment.user_id === currentUserId ? 'flex flex-col items-end' :
                                        'flex flex-col items-start'">
                                    <div class="flex items-center gap-2 mb-1">
                                        <span class="text-[10px] font-bold text-gray-500"
                                            x-text="comment.user.username"></span>
                                        <span class="text-[9px] text-gray-400"
                                            x-text="new Date(comment.created_at).toLocaleString('id-ID', {hour:'2-digit', minute:'2-digit'})"></span>
                                    </div>
                        ...
                        <template x-if="isResolved">
                            <div
                                class="absolute inset-0 bg-gray-50/60 backdrop-blur-[1px] flex items-center justify-center z-10">
                                <div
                                    class="bg-success text-white px-6 py-2 rounded-full shadow-lg flex items-center gap-2 border-2 border-white/20">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    <span class="text-sm font-bold uppercase tracking-wider">Laporan Sudah
                                        Selesai</span>
                                </div>
                            </div>
                        </template>
                        ...
                        <div x-show="!isResolved" class="p-3 bg-white border-t border-gray-100">
                            <div class="relative">
                                <textarea x-model="newComment" @keydown.enter.prevent="postComment()" placeholder="Tulis tanggapan..."
                                    rows="1"
                                    ...
                                    x-init="$el.style.height = '40px';
                                    $watch('newComment', val => {
                                        $el.style.height = '40px';
                                        $el.style.height = $el.scrollHeight + 'px'
                                    })"></textarea>
                                <button @click="postComment()" :disabled="!newComment.trim() || isSubmitting"
                                    ...
                                    <svg x-show="isSubmitting" class="animate-spin h-5 w-5" viewBox="0 0 24 24">
                            ...
                            <p class="text-[9px] text-gray-400 mt-2 px-1">Tanggapan hanya dapat dilihat oleh Anda dan
                                Admin.</p>
                        </div>
                    </div>
                ...
                <!-- Modal Footer -->
                <div
                    class="px-6 py-4 bg-gray-100/50 border-t border-gray-100 flex justify-between items-center gap-4 flex-wrap">
